    ...
    listForums: function() {
		...
		if(settings) {
			...
			for(var cnt = 0; cnt< settings.length; cnt++) {
				...
				prepList = prepList + "<ons-list-item>" + innerThis.ellipse(settings[cnt].forum, 27) + " <div class='right'><ons-icon icon='md-link' class='list__item__icon' onclick='app.register(\"" + settings[cnt].forum + "\", null);'></ons-icon>&nbsp;&nbsp;<ons-icon icon='md-delete' class='list__item__icon' onclick='app.deleteForum(" + cnt + ");'></ons-icon></div></ons-list-item>";
    ...
    displayForumNames: function() {
    	//Call this during initialisation on app startup
    		...
    		for(var cnt = 0; cnt< settings.length; cnt++) {
    			...
    			prepList = prepList + "<a class=\"aj-list\" href=\"" + encodeURI(settings[cnt].url) + "\" target=\"_blank\"><ons-list-item>" + innerThis.ellipse(settings[cnt].forum, 27) + "</ons-list-item></a>";  //</ons-list-item>
    ...
    checkTransitioningData: function() {
    	...
    	if(!transitioned) {
    		...
    		if(localStorage.getItem("registrationId")) {
				...
				var items = [ "pollingURL", "gotoforum", "api", "loggedUser", "registrationId", "settings"];
